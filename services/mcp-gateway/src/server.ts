...
app.post("/mcp/execute", authMiddleware, rateLimitMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    if (!config.executeEnabled) {
      res.status(503).json({ error: "EXECUTE_DISABLED", message: "Execute endpoint is disabled." });
      return;
    }
