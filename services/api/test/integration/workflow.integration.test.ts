...
const testTimeoutMs = Number(
  process.env.INTEGRATION_TEST_TIMEOUT_MS || String(jobTimeoutMs + 20000)
);
const integrationAuthTokenSecret =
  process.env.INTEGRATION_AUTH_TOKEN_SECRET ||
...
describe.skipIf(!shouldRun)('integration workflow', () => {
  it(
    'processes upload -> job -> status -> cleanup with idempotency and deletion checks',
    async () => {
      ...
      const uploadFormData = buildUploadFormData(
      ...
      while (Date.now() < deadline) {
        const statusResponse = await fetch(
          `${apiBaseUrl}/api/jobs/${encodeURIComponent(createdJob.id)}`,
          {
            headers: authHeaders,
          }
        );
      ...
      const createJobFromDeletedInputPayload = await readJson<{ error: string }>(
        createJobFromDeletedInputResponse
      );
