...
describe.skipIf(!shouldRun)('integration auth+billing+dedup', () => {
  it('upgrades plan through billing webhook and deduplicates repeated uploads', async () => {
    ...
    const signature = createHmac('sha256', billingWebhookSecret)
    ...
    const profileResponse = await fetch(
      `${apiBaseUrl}/api/auth/session/${encodeURIComponent(subjectId)}`
    );
    ...
    const firstCompletePayload = await readJson<{
    ...
    const jobCreatePayload = await readJson<{
      inputObjectKey: string;
      quota: { usedCount: number };
    }>(jobCreateResponse);
