...
describe("cleanup idempotency and ttl sweep", () => {
  ...
  it("handles empty objectKeys array", async () => {
    const { base } = await startTestServer(() => new Date("2026-02-23T00:00:00.000Z"));
    ...
    const cleanup = await fetch(`${base}/api/cleanup`, {
      ...
      headers: {
        "content-type": "application/json",
        "idempotency-key": "cleanup-empty-key"
      },
    ...
    expect(payload.notFound).toBe(0);
  });

  it("normalizes objectKeys by removing duplicates and sorting", async () => {
    const { base } = await startTestServer(() => new Date("2026-02-23T00:00:00.000Z"));
    const uploadA = await initUpload(base, "seller_normalize_1", "a.jpg");
    const uploadB = await initUpload(base, "seller_normalize_1", "b.jpg");

    const first = await fetch(`${base}/api/cleanup`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "idempotency-key": "cleanup-normalize-key"
      },
      body: JSON.stringify({
        objectKeys: [uploadB.objectKey, uploadA.objectKey, uploadB.objectKey]
      })
    });
    expect(first.status).toBe(202);
    ...
    const second = await fetch(`${base}/api/cleanup`, {
      ...
      body: JSON.stringify({
        objectKeys: [uploadA.objectKey, uploadB.objectKey]
      })
    });
  ...
  it("records not_found audit entries for missing keys", async () => {
    ...
    const cleanup = await fetch(`${base}/api/cleanup`, {
      ...
      headers: {
        "content-type": "application/json",
        "idempotency-key": "cleanup-notfound-key"
      },
      body: JSON.stringify({ objectKeys: ["tmp/missing/key1.jpg", "tmp/missing/key2.jpg"] })
    });

    expect(cleanup.status).toBe(202);
    const payload = await cleanup.json();
    expect(payload.cleaned).toBe(0);
    expect(payload.notFound).toBe(2);

    const audits = await fetch(`${base}/api/internal/deletion-audit?limit=10`, {
      headers: { "x-worker-token": workerToken }
    });
    const auditPayload = await audits.json();
    const notFoundEntries = auditPayload.items.filter(
      (item: { reason: string; result: string }) => item.reason === "page_exit" && item.result === "not_found"
    );
    expect(notFoundEntries.length).toBeGreaterThanOrEqual(2);
  });

  it("reports both cleaned and notFound counts", async () => {
    const { base } = await startTestServer(() => new Date("2026-02-23T00:00:00.000Z"));
    const uploadA = await initUpload(base, "seller_mixed", "a.jpg");
    const uploadB = await initUpload(base, "seller_mixed", "b.jpg");

    const cleanup = await fetch(`${base}/api/cleanup`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "idempotency-key": "cleanup-mixed-key"
      },
      body: JSON.stringify({
        objectKeys: [uploadA.objectKey, "tmp/missing/x.jpg", uploadB.objectKey, "tmp/missing/y.jpg"]
      })
    });

    expect(cleanup.status).toBe(202);
    const payload = await cleanup.json();
    expect(payload.cleaned).toBe(2);
    expect(payload.notFound).toBe(2);
  });

  it("handles non-array objectKeys gracefully", async () => {
    const { base } = await startTestServer(() => new Date("2026-02-23T00:00:00.000Z"));
    ...
    const cleanup = await fetch(`${base}/api/cleanup`, {
      ...
      headers: {
        "content-type": "application/json",
        "idempotency-key": "cleanup-invalid-key"
      },
      body: JSON.stringify({ objectKeys: "not-an-array" })
    });
  ...
  it("prunes expired uploads before cleanup", async () => {
    let currentTime = new Date("2026-02-23T00:00:00.000Z");
    const now = () => currentTime;
    const { base } = await startTestServer(now);
    const uploadA = await initUpload(base, "seller_prune", "a.jpg");
    const uploadB = await initUpload(base, "seller_prune", "b.jpg");
    ...
    const cleanup = await fetch(`${base}/api/cleanup`, {
      ...
      headers: {
        "content-type": "application/json",
        "idempotency-key": "cleanup-prune-key"
      },
      body: JSON.stringify({ objectKeys: [uploadA.objectKey, uploadB.objectKey] })
    });

    expect(cleanup.status).toBe(202);
    const payload = await cleanup.json();
    expect(payload.expiredPurged).toBe(2);
    expect(payload.notFound).toBe(2);
  });

  it("requires worker token for sweep endpoint", async () => {
    const { base } = await startTestServer(() => new Date("2026-02-23T00:00:00.000Z"));
    ...
    const payload = await sweep.json();
    expect(payload.error).toBe("UNAUTHORIZED_WORKER");
  });

  it("requires worker token for deletion audit endpoint", async () => {
    const { base } = await startTestServer(() => new Date("2026-02-23T00:00:00.000Z"));

    const audits = await fetch(`${base}/api/internal/deletion-audit`);

    expect(audits.status).toBe(401);
    const payload = await audits.json();
    expect(payload.error).toBe("UNAUTHORIZED_WORKER");
  });

  it("limits deletion audit query results", async () => {
    const { base, workerToken } = await startTestServer(() => new Date("2026-02-23T00:00:00.000Z"));
    ...
    for (let i = 0; i < 25; i++) {
      await fetch(`${base}/api/cleanup`, {
        ...
        headers: {
          "content-type": "application/json",
          "idempotency-key": `cleanup-audit-${i}`
        },
        body: JSON.stringify({ objectKeys: [`tmp/test/${i}.jpg`] })
      });
    }

    const audits = await fetch(`${base}/api/internal/deletion-audit?limit=5`, {
      headers: { "x-worker-token": workerToken }
    });
