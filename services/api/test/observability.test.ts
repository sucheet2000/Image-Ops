...
describe('observability routes', () => {
  ...
  it('returns degraded when a dependency probe fails', async () => {
    const config = createTestConfig();
    const services = createFakeServices();
    vi.spyOn(services.storage, 'headObject').mockRejectedValueOnce(
    ...
    try {
      ...
      try {
        ...
        expect(unauthorizedResponse.headers.get('www-authenticate')).toBe(
          'Bearer realm="metrics", error="invalid_token"'
        );
        ...
        expect(metrics).toContain(
          'image_ops_http_requests_total{method="GET",path="/health",status_code="200"} 1'
        );
        expect(metrics).toContain(
          'image_ops_http_requests_total{method="GET",path="/ready",status_code="200"} 1'
        );
  ...
  it('filters watch tower logs by severity', async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${server.baseUrl}/api/observability/logs?level=error&limit=10`,
        {
          headers: { ...bearerAuthHeaders('observer_1') },
        }
      );
