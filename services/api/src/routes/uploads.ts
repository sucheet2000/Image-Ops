    ...
    asyncHandler(async (req, res) => {
      ...
      if (!SUPPORTED_MIME.has(mime)) {
        res.status(400).json({
          ...
          message: 'Supported MIME types: image/jpeg, image/png, image/webp',
        });
        return;
      }
      ...
      const expiresAt = new Date(
        deps.now().getTime() + deps.config.signedUploadTtlSeconds * 1000
      ).toISOString();
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!parsed.success) {
        res
          .status(400)
          .json({ error: 'INVALID_UPLOAD_COMPLETE_REQUEST', details: parsed.error.flatten() });
        return;
      }
      ...
      if (!parsedKey) {
        res.status(400).json({
          ...
          message: 'Object key does not match expected upload key format.',
        });
        return;
      }
      ...
      if (!payload.objectKey.startsWith(`${expectedPrefix}/`)) {
        res.status(400).json({
          error: 'INVALID_OBJECT_KEY',
          message: 'Object key does not match subject upload prefix.',
        });
        return;
      }
      ...
      if (!head.exists) {
        res.status(404).json({
          ...
          message: 'Input object key is missing or expired.',
        });
        return;
      }
      if (
        typeof head.contentLength === 'number' &&
        head.contentLength > deps.config.maxUploadBytes
      ) {
        res.status(413).json({
          error: 'FILE_TOO_LARGE',
          message: `Maximum upload size is ${deps.config.maxUploadBytes} bytes.`,
