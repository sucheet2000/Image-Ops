...
function sameSiteCookieValue(
  value: ApiConfig['authRefreshCookieSameSite']
): 'Lax' | 'Strict' | 'None' {
  if (value === 'strict') {
    return 'Strict';
  }
...
export function registerAuthRoutes(
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const subjectId = toSafeSubjectId(
        parsed.data.subjectId || `session_${ulid(deps.now().getTime())}`
      );
      ...
      if (parsed.data.plan) {
        if (!isPlan(parsed.data.plan)) {
          res
            .status(400)
            .json({ error: 'INVALID_PLAN', message: 'Plan must be free, pro, or team.' });
          return;
        }
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!parsed.success) {
        res
          .status(400)
          .json({ error: 'INVALID_GOOGLE_AUTH_REQUEST', details: parsed.error.flatten() });
        return;
      }
      ...
      try {
        ...
        if (
          ...
          (error instanceof Error &&
            /(google|id token|audience|subject|expired|invalid)/i.test(error.message))
        ) {
          res.status(401).json({
      ...
      res.setHeader(
        'set-cookie',
        serializeRefreshCookie(deps.config, refreshIssued.token, deps.config.authRefreshTtlSeconds)
      );
      res.status(200).json({
        token,
        tokenType: 'Bearer',
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      res.setHeader(
        'set-cookie',
        serializeRefreshCookie(deps.config, rotated.token, deps.config.authRefreshTtlSeconds)
      );
      res.status(200).json({
        token,
        tokenType: 'Bearer',
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!profile) {
        res
          .status(404)
          .json({ error: 'SESSION_NOT_FOUND', message: 'Subject profile does not exist.' });
        return;
      }
