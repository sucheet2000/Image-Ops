...
CURL_MAX_TIME_STANDARD=30
CURL_MAX_TIME_POLL=10

echo "==> health check"
curl --connect-timeout "${CURL_CONNECT_TIMEOUT}" --max-time "${CURL_MAX_TIME_STANDARD}" -fsS "${BASE_URL}/health" >/dev/null
...
echo "==> init upload"
UPLOAD_INIT_RESPONSE="$(curl --connect-timeout "${CURL_CONNECT_TIMEOUT}" --max-time "${CURL_MAX_TIME_STANDARD}" -fsS -X POST "${BASE_URL}/api/uploads/init" \
...
echo "==> upload object"
UPLOAD_STATUS="$(curl --connect-timeout "${CURL_CONNECT_TIMEOUT}" --max-time "${CURL_MAX_TIME_STANDARD}" -sS -o /dev/null -w "%{http_code}" -X PUT "${UPLOAD_URL}" \
...
curl --connect-timeout "${CURL_CONNECT_TIMEOUT}" --max-time "${CURL_MAX_TIME_STANDARD}" -fsS -X POST "${BASE_URL}/api/uploads/complete" \
  -H "content-type: application/json" \
  "${AUTH_HEADER[@]}" \
  -d "{\"subjectId\":\"${SUBJECT_ID}\",\"objectKey\":\"${OBJECT_KEY}\"}" >/dev/null

echo "==> create job (requires auth when API_AUTH_REQUIRED=true)"
CREATE_JOB_STATUS="$(curl --connect-timeout "${CURL_CONNECT_TIMEOUT}" --max-time "${CURL_MAX_TIME_STANDARD}" -sS -o "${TMP_JOB}" -w "%{http_code}" -X POST "${BASE_URL}/api/jobs" \
  -H "content-type: application/json" \
  "${AUTH_HEADER[@]}" \
...
while [[ "$(date +%s)" -lt "${DEADLINE}" ]]; do
  STATUS_JSON="$(curl --connect-timeout "${CURL_CONNECT_TIMEOUT}" --max-time "${CURL_MAX_TIME_POLL}" -fsS -X GET "${BASE_URL}/api/jobs/${JOB_ID}" "${AUTH_HEADER[@]}")"
...
curl --connect-timeout "${CURL_CONNECT_TIMEOUT}" --max-time "${CURL_MAX_TIME_STANDARD}" -fsS "${DOWNLOAD_URL}" >/dev/null

echo "==> cleanup"
CLEANUP_STATUS="$(curl --connect-timeout "${CURL_CONNECT_TIMEOUT}" --max-time "${CURL_MAX_TIME_STANDARD}" -sS -o "${TMP_CLEANUP}" -w "%{http_code}" -X POST "${BASE_URL}/api/cleanup" \
  -H "content-type: application/json" \
