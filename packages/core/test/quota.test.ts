...
describe('quota', () => {
  it('allows up to limit within active window', () => {
    ...
    const result = applyQuota(
      { windowStartAt: now.toISOString(), usedCount: FREE_PLAN_LIMIT - 1 },
      1,
      now
    );
  ...
  it('blocks requests exceeding free limit', () => {
    const now = new Date('2026-02-23T00:00:00.000Z');
    const result = applyQuota(
      { windowStartAt: now.toISOString(), usedCount: FREE_PLAN_LIMIT },
      1,
      now
    );
  ...
  it('resets quota after 10-hour window expires', () => {
    ...
    const result = applyQuota(
      { windowStartAt: start.toISOString(), usedCount: FREE_PLAN_LIMIT },
      1,
      now
    );
  ...
  it('rejects negative requestedImages', () => {
    const now = new Date('2026-02-23T00:00:00.000Z');
    expect(() => applyQuota({ windowStartAt: now.toISOString(), usedCount: 1 }, -1, now)).toThrow(
      'requestedImages must be non-negative'
    );
  });
...
describe('structured log privacy guards', () => {
  it('redacts secret-like keys and signed-url query values', () => {
    ...
    expect(payload.uploadUrl).toBe(
      'https://example.test/file?X-Amz-Signature=[REDACTED]&token=[REDACTED]'
    );
