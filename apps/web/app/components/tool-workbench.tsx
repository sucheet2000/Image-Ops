  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      try {
        ...
        if (!cancelled) {
          setErrorText(
            `Failed to initialize viewer session: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
    };
  ...
  const runPipeline = async () => {
    ...
    try {
      ...
      if (!uploadInitResponse.ok) {
        ...
        throw new Error(
          readErrorMessage(payload, `Upload init failed (${uploadInitResponse.status})`)
        );
      }
      ...
      if (!uploadPostResponse.ok) {
        throw new Error(
          `Upload POST failed (${uploadPostResponse.status}). Verify storage endpoint configuration.`
        );
      }
      ...
      if (!uploadCompleteResponse.ok) {
        ...
        throw new Error(
          readErrorMessage(payload, `Upload complete failed (${uploadCompleteResponse.status})`)
        );
      }
      ...
      if (!jobCreateResponse.ok) {
        ...
        throw new Error(
          readErrorMessage(payload, `Job create failed (${jobCreateResponse.status})`)
        );
      }
      ...
      while (Date.now() < deadline) {
        const statusResponse = await apiFetch(`${apiBaseUrl}/api/jobs/${created.id}`, {
          method: 'GET',
        });
        ...
        <div className="workbench-grid">
          <div className="workbench-form">
              ...
              <div className="field-grid" style={{ marginTop: '1rem' }}>
                ...
                <div className="field">
                  ...
                  <select
                    id="resize-fit"
                    value={resizeFit}
                    onChange={(event) => setResizeFit(event.target.value as ResizeFit)}
                  >
            ...
            <div className="workbench-actions">
              <button
                type="button"
                className="editorial-button accent btn-primary"
                disabled={running || !file || !subjectId}
          ...
          <aside className="workbench-output">
            <AnimatePresence mode="wait">
              ...
              >
                <span className={`status-chip ${chip.className} status-${chip.className}`}>
                  {chip.label}
                </span>
                ...
                {result?.downloadUrl ? (
                  <div className="tool-result" style={{ marginTop: '1rem' }}>
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="editorial-button primary btn-primary"
                    >
