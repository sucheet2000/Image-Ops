...
export function WatchTowerShell() {
  ...
  const loadLogs = useCallback(async (silent: boolean) => {
    ...
    try {
      ...
      const response = await apiFetch(`${apiBaseUrl}/api/observability/logs?${params.toString()}`, { method: "GET" });
      if (!response.ok) {
    ...
    <main className="app-page">
      <section className="page-shell">
        <header className="page-head">
          <span className="section-label reveal-el" data-delay="0">Watch Tower</span>
          ...
          <p className="reveal-el" data-delay="200">
            Filter by severity, isolate failing events, and inspect structured payloads while incidents are active.
          </p>
        </header>
        ...
        <section className="dashboard-layout reveal-el" data-delay="280">
          <aside className="dashboard-sidebar">
            ...
            <nav className="dashboard-nav" aria-label="Dashboard navigation" style={{ marginTop: "0.9rem" }}>
              ...
              <Link href="/dashboard/watchtower" className="active">Watch Tower</Link>
                  ...
                  logs.map((entry) => (
                    <li key={entry.id} className="watchtower-log-item">
                      <div className="watchtower-log-header">
                        ...
                        <span className={`status-chip ${entry.level === "error" ? "failed" : "info"}`}>
